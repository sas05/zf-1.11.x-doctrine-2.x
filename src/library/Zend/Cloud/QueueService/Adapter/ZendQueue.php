    ...
    extends Zend_Cloud_QueueService_Adapter_AbstractAdapter
{
    ...
    {
        try {
            $this->_queues[$name] = $this->_queue->createQueue($name, isset($options[Zend_Queue::TIMEOUT])
                                                                            ? $options[Zend_Queue::TIMEOUT] : null);
            return $name;
        } catch (Zend_Queue_Exception $e) {
    ...
    public function receiveMessages($queueId, $max = 1, $options = null)
    {
        ...
        try {
            $res = $this->_queues[$queueId]->receive($max, isset($options[Zend_Queue::TIMEOUT])
                                                                 ? $options[Zend_Queue::TIMEOUT] : null);
