...
class Zend_Pdf_Outline_Loaded extends Zend_Pdf_Outline
{
        ...
        SplObjectStorage $processedOutlines = null)
    {
        ...
        if ($updateChildNavigation) {
            ...
            if (count($this->childOutlines) != 0) {
                $this->_outlineDictionary->Count = new Zend_Pdf_Element_Numeric(($this->isOpen() ? 1
                            : -1) * count($this->childOutlines));
            } else {
    ...
    {
        printf(":%3d:%s:%s:%s%s  :\n", count($this->childOutlines), $this->isItalic() ? 'i' : ' ', $this->isBold() ? 'b'
